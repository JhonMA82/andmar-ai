...
9. Work unit execution, cadence, and validation:
   ...
   - Block an active unit with `.../work-ledger-lifecycle.mjs block .andmar/work/<work-id> WU-N --reason "..."`; resume it only with `resume ... --reason "why the blocker is resolved"`. Reopen a done unit only when its outcome/evidence became invalid, using `reopen ... --reason "..."`; reopening clears the unit's current evidence pointer so it cannot masquerade as still proven. Completed Ledgers are immutable to this helper.
   - Use `.../work-ledger-lifecycle.mjs status .andmar/work/<work-id>` to recover compact unit state when useful. The helper validates before and after mutation and rolls back a transition that would make the Ledger structurally invalid.
   ...
   - Validate the structural integrity of the Work Ledger with `node "${OPENCODE_CONFIG_DIR:-$HOME/.config/opencode}/plugins/andmar-ai/scripts/validate-work-ledger.mjs" .andmar/work/<work-id>` after initialization, steering that modifies requirements, material work-unit list changes, and preparing completion. Lifecycle transitions already validate atomically; do not redundantly run the validator after every lifecycle command.
